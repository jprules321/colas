...
/* Normal colors */
#if 0
static char *Black   = "[1m[30m";
static char *Red     = "[1m[31m";
static char *Green   = "[1m[32m";
static char *Yellow  = "[1m[33m";
static char *Blue    = "[1m[34m";
static char *Magenta = "[1m[35m";
static char *Cyan    = "[1m[36m";
static char *White   = "[1m[37m";
#endif
...
char *gatxtl(char *str, gaint level) { /* colorize according to level */
  ...
  if ( scheme==0 ) {
    ...
    if (level==3 ) return gatxts(str,"cyan");
  }
  else if ( scheme==1 ) {
    if (level==-1) return gatxts(str,"Green"); /* prompt */
    if (level==0)  return gatxts(str,"Red");
