...
/*ams Provides access to internal metafile buffer to UDXs.
      Just like fread(), but read metafile buffer instead; on
      input *stream is not used, but kept for keeping same
      arguments as fread(). ams*/

static short *udx_poi, *udx_pend;    /*ams User Defined Extension API ams*/
static short udx_cnt, udx_flag, udx_ib, udx_phase;  
...
size_t udx_mread (short *ptr, size_t size, size_t nmemb, FILE *stream) {
  ...
  if ( size == -1 ) {
    ...
    if (dbmode && pntf==0) {
      lens2[pnt2-1] = hpnt-hbuff;
      udx_cnt = pnt2; udx_flag = 1;
    } else {  
      lens[pnt-1] = hpnt-hbuff;
      udx_cnt = pnt; udx_flag = 0;
    }
    if ( udx_cnt < 1 ) {
      udx_poi  = (short *) NULL;
      udx_pend = (short *) NULL;
      ...
      udx_ib = 0;
      if (udx_flag) {
	udx_poi =  bufs2[udx_ib];
	udx_pend = udx_poi + lens2[udx_ib];
      } else { 
	udx_poi = bufs[udx_ib];
	udx_pend = udx_poi + lens[udx_ib];
      } 
    } 
    return 0;
  }
  
  /* simulate "Start of Plotting" */  
  if ( udx_phase == 0 ) {
    *ptr = -1; 
    udx_phase = 1;

  /* arguments for cmd=-1: plot size */
  } else if ( udx_phase == 1 ) {  
    xx = (int)(xrsize*1000.0+0.5);
    if (xx<0) xx=0;
    if (xx>32760) xx=32760;
    ptr[0] = xx;
    xx = (int)(yrsize*1000.0+0.5);
    if (xx<0) xx=0;
    if (xx>32760) xx=32760;
    ptr[1] = xx;
    udx_phase = 2;

  /* Return coentents of metafile memory buffer */
  } else if ( udx_poi<udx_pend ) {
    for ( i=0; i<nmemb; i++ ) {
          ptr[i] = *udx_poi;
          udx_poi++;
    }
    if ( udx_poi >= udx_pend ) {
         udx_ib++;                    /* get next memory buffer */
         if ( udx_ib < udx_cnt ) {
              if (udx_flag) {
                  udx_poi =  bufs2[udx_ib];
                  udx_pend = udx_poi + lens2[udx_ib];
  ...
  /* simulate "End of Page" */ 
  } else if ( udx_phase == 2 ) {
    *ptr = -2; 
    udx_phase = 3;

  /* simulate "End of Plotting" */ 
  } else if ( udx_phase == 3 ) {
    *ptr = -9; 
  }
