  ...
  while (cont) {
    ...
    /* Set color */
      ...
      if (cflag) {
        ...
        snprintf(ch,19," %.5g %.5g %.5g c",red[lcolor],green[lcolor],blue[lcolor]);
      } else {
        if (lcolor>0) snprintf(ch,19," %.5g g",grey[1]);
        else snprintf(ch,19," %.5g g",grey[0]);
      }
      mycln(ch,&len);
      chout(ch,len);
      fflag = 1;
	...
	xycnv (opts[1],opts[3],&xhi,&yhi);
      ...
      if (!cflag) {
        if (grey[lcolor]<-100.0) snprintf(ch,19," %.5g g",grey[15]);
        else snprintf(ch,19," %.5g g",grey[lcolor]);
        mycln(ch,&len);
        chout(ch,len);
      }
      chout(" n",2);
      snprintf(ch,19," %.5g %.5g m",xlo,ylo);
      mycln(ch,&len);
      chout(ch,len);
      snprintf(ch,19," %.5g %.5g d",xhi,ylo);
      mycln(ch,&len);
      chout(ch,len);
      snprintf(ch,19," %.5g %.5g d",xhi,yhi);
      mycln(ch,&len);
      chout(ch,len);
      snprintf(ch,19," %.5g %.5g d",xlo,yhi);
      mycln(ch,&len);
      chout(ch,len);
      snprintf(ch,19," %.5g %.5g d",xlo,ylo);
      mycln(ch,&len);
      chout(ch,len);
      chout(" f",2);
      if (!cflag) {
    ...
    /* Draw to */
      ...
      if (!sflag) {
        chout (" n",2);         
        snprintf(ch,19," %.5g %.5g m",xpos,ypos);
        mycln(ch,&len);
        chout(ch,len);
        scnt = 0;
      }
	...
	xycnv (opts[0],opts[1],&xpos,&ypos);
      ...
      if (scnt>511 && !filflg) {    /* Keep vector counts small */
        snprintf(ch,19," %.5g %.5g d",xpos,ypos);
        mycln(ch,&len);
        chout(ch,len);
        chout (" s n",4);
        snprintf(ch,19," %.5g %.5g m",xpos,ypos);
