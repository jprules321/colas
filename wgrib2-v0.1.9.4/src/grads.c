...
/* These are NestedVM functions for interfacing to the Java subsystem */
#ifdef __nestedvm__
extern void _pause();
#endif
...
#ifdef __nestedvm__
    gxflag(0);  /* save rc for Java subsystem */
    _pause();   /* give control to Java */
#else
...
printf("          -C N    Enables colorized text with color scheme N\n");
printf("\n   Example:\n");
...
/*
    Function wrappers eliminating dependency on internal state (gcmn).
    Needed by the Java interface.
*/
int gaCmd ( char *action ) __attribute__((section(".text")));
int gaCmd ( char *action )
{
  return gacmd(action,&gcmn,0);
}

void gxHdrw () __attribute__((section(".text")));
void gxHdrw ()
{
  gxhdrw(gcmn.dbflg);
}
