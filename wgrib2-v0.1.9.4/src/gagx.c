...
#endif
	  else if (pcm->gout2a == 14) gacntr (pcm,3);
	  else gacntr (pcm,2);
	} 
	else {
...
void gagtif (struct gacmn *pcm, gaint kmlflg) {
 ...
 /* Open output files */
 if (kmlflg) { 
   /* For gxout KML, open the file for the image output, we'll open the KML file later */
   if (pcm->tifname)
...
gaint write_kml(struct gacmn *pcm, gadouble *tiepoints) {
 FILE *kmlfp=NULL;
 struct gagrid *pgr;
 ...
 if ((tiepoints[0+3]==tiepoints[6+3]) && (tiepoints[12+3]==tiepoints[18+3])) {
   if (tiepoints[0+3]<tiepoints[12+3])
     snprintf(pout,255,"   <west>%15.10g</west>\n   <east>%15.10g</east>\n",
 ...
 if ((tiepoints[0+4]==tiepoints[12+4]) && (tiepoints[6+4]==tiepoints[18+4])) {
   if (tiepoints[0+4]<tiepoints[12+4])
     snprintf(pout,255,"   <south>%15.10g</south>\n   <north>%15.10g</north>\n",
...
/* filflg:  0 - line contours, 1 - shaded contours, 2 - grfill, 3 - imap.  */
  ...
  if (filflg) {
	...
	if (pcm->ccolor > 0 && pcm->ccflg == 0) cntrcol = pcm->ccolor; /* overlays */
	  ...
	  snprintf(chlab,49,"%g",rr);
        ...
        if (pcm->clab==3) scntr.mask=1; else scntr.mask=0;
        if (smooth) {
...
void gaimap (gadouble *r, gaint is, gaint js, gadouble *vs, gaint *clrs, gaint lvs, char *ru, gadouble xlo, gadouble xhi, gadouble ylo, gadouble yhi) {
gadouble x,y,*v,*xvals,*yvals,xscl,yscl;
