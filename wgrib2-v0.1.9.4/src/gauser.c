...
static char *ccdims[5] = {"Xdim","Ydim","Zdim","Tdim","Val"};    /* JMA fix this for Edim */
...
gadouble minvals[4], maxvals[4];
  ...
  if (arg == NULL) {
    ...
    gaprnt (2,"  q shp      Returns information about a shapefile\n");
...
size_t sz;
static char *kwds[121] = {"X","Y","Z","T","LON","LAT","LEV","TIME",
  ...
  if ((cmd = nxtwrd (cmd))==NULL) {
    ...
    *(ch2+i1+4) ='\0';           /* ch2 contains the filename root plus ".kml" */
    ...
    snprintf(pout,255,"%s (for the TIFF image) \n",pcm->tifname);
    gaprnt (2,pout);
    snprintf(pout,255,"%s (for the KML text file) \n",pcm->kmlname);
    gaprnt (2,pout);
#else
...
char *filename={"grads.sdfwrite.nc"};
  ...
  */
   ...
   */
  if (pcm->sdfprec==8) {
    rc = nc_put_att_double(pcm->ncwid, varid, "missing_value", NC_DOUBLE, 1, &pcm->undef);
  }
  else if (pcm->sdfprec==4) {
    flundef = (gafloat)pcm->undef;
    rc = nc_put_att_float(pcm->ncwid, varid, "missing_value", NC_FLOAT, 1, &flundef);
  }
  if (rc) {
