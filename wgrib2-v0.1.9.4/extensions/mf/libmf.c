...
/* output array */
  ...
  if(argc == 1) {
      ...
      printf(" for method=\'GR\' :: Mike Fiorino GRid routines ; methopts = maxmin sortby pcntile latc lonc\n\n");
      ...
      printf(" for method=\'CL\' :: NCEP CLosed routines ; methopts = maxmin rad cintinf latc lonc\n\n");
      ...
      printf(" for method=\'TM\' :: Tim Marchok center locator using barnes analysis ; methopts = maxmin guesslat guesslon\n\n");
...
/* -- run the fortran code -- */
  ...
  if(docl || dogr) {
      ...
      if(dogr)
	  sprintf(pout,"%s GR method N: %d  maxmin: %d  pcntile: %6.0f  latc: %6.2f lonc: %7.2f nsigdig: %d\n",argv[0],nrr,maxmin,pcntile,latc,lonc,nsigdig);
...
void ftnuv2trw_ ( gaData_t ua[], gaData_t va[], gaData_t lon[], gaData_t lat[], gaData_t dum [], gaData_t *undef, 
		   int *im, int *jm, gaData_t *centlon, gaData_t *centlat, 
		   int *iout, int *rc );
...
int f_uv2trw (struct gafunc *pfc, struct gastat *pst) {
	...
	gaprnt(0,pout);
	gaprnt(0,"          Usage:  uv2trw(u,v,[iout],[lon,lat]))\n\n");
	gaprnt(0,"returns the tangential velocity (iout=1; the default) relative to a fixed lon/lat\n");
	gaprnt(0,"if no lon/lat is provided, then the center of the grid is assumed the fixed point\n");
	gaprnt(0,"Examples:\n");
	gaprnt(0,"   ga-> d uv2trw(ua,va,-1)    (return the radial wind\n\n");
	return(1);
    }
...
/* 2nd grid */
  ...
  if(imu != imv || jmu != jmu) {
    ...
    ftnuv2trw_ ( u, v, lon, lat, dum, &undef,
		  &im, &jm, &centlon, &centlat,
		  &iout, &rc );
...
B<tcprop> I<EXPR> I<LON> I<LAT> I<RADIUS> - Properties relative to lon/lat
...
=head2 B<tcprop> I<EXPR> I<LON> I<LAT> I<RADIUS> I<BEARING> 
...
I<LATC> - set the latitude for distance [nm] calculations

I<LONC> - set the longitude (deg E) for distance [nm] calculations
...
then the center of the grid is assumed the  fixed point. For example, 
