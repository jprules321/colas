...
void ftntrack_ (double lons[], double lats[], int *nobs, int *nmax,
                  int *sat, int nymd[], int nhms[], int *dt, int *rc );

void ftnmasking_ (double field[], int *nlon, int *nlat, 
                  double lons[], double lats[], double *undef, 
                  int *sat, int nymd[], int nhms[], int *dt, 
...
int c_track ( int argc, char **argv, struct gacmn *pcm) {
  ...
  if (argc<2) {
    sprintf(pout,"\nError from %s: Too few args \n\n", name);
    gaprnt(0,pout);
    sprintf(pout,"          Usage:  %s sat\n",name);
    gaprnt(0,pout);
    sprintf(pout,"        Example:  %s aqua \n\n",name);
    gaprnt(0,pout);
    return(1);
  }
  ...
  if (*(pcm->pfid->grvals[3]+5)!=0) { /* monthly data not supported for now */
   ...
   nhms[0] = ta.hr * 10000 + ta.mn;
   nhms[1] = tb.hr * 10000 + tb.mn;
  ...
  lowcas(sat_name);
   ...
   gaprnt (2,pout);
    ...
    ftntrack_ (lons,lats, &nobs, &nmax, &sat, nymd, nhms, &dt, &rc );
    if ( rc ) {
...
int f_mask (struct gafunc *pfc, struct gastat *pst) {
  ...
  if (pfc->argnum<2) {
    ...
    gaprnt(0,pout);
    sprintf(pout,"          Usage:  %s(expr,sat[,swath_left[,swath_right[,ds]])\n",name);
    gaprnt(0,pout);
    sprintf(pout,"        Examples:  %s(ts,aqua)\n",name);
    gaprnt(0,pout);
    sprintf(pout,"                   %s(ts,aqua,400)\n",name);
    gaprnt(0,pout);
    sprintf(pout,"                   %s(ts,aqua,300,500,10)\n",name);
    gaprnt(0,pout);
    return(1);
  }
 ...
 if (*(pst->pfid->grvals[3]+5)!=0) { /* monthly data not supported for now */
  ...
  if (pst->type==1) {  /* gridded data */
    ...
    ftnmasking_ ( val, &im, &jm, lon, lat, &undef, 
